...
const[cast,setCast]=useState([])
    async function getcast(id){
       let results=await axios.get(`https://api.themoviedb.org/3/tv/${id}/credits?api_key=d687519299d61c8a9d38cca590708195&language&language=en-US`) 
       setCast(results.data.cast)
        console.log(results.data.cast);         }
const[similar,setSimilar]=useState([])
async function getSimilar(id){
   let {data}=await axios.get(`https://api.themoviedb.org/3/tv/${id}/similar?api_key=d687519299d61c8a9d38cca590708195&language=en-US&page=1`) 
...
<div className="col-md-3">
        ...
        {movieDetails?.homepage?<button className='btn btn-warning w-100 mt-3'><a href={movieDetails.homepage} target="_blank">Show page</a></button>:null}
    </div>
    <div className="col-md-9">
        <div className='MovieDetailsTitle p-3'>
            <h2 className='h3 '>{movieDetails.name}</h2>
    ...
    <div className=' lang'>
        ...
        <div className=' d-flex'>
        <li className='h5'>Epsodes:</li>
          <p className='m-2 mt-0 mb-0 text-muted'>{movieDetails.number_of_episodes}</p>
